...
- PR corpus under `decomp-orchestrator/knowledge/sources/past_prs/data/current`
- searchable per-PR JSON library under `decomp-orchestrator/knowledge/sources/past_prs/data/prs`
...
```bash
rg -n "FunctionName" src config/GALE01/symbols.txt decomp-orchestrator/knowledge/sources/past_prs/data/current/analysis decomp-orchestrator/knowledge/sources/past_prs/data/prs
```
...
what the original developer would likely have called related fields, callbacks,
states, helper functions, data tables, and sibling files. Use local source,
headers, symbols, splits, PR records, and resource indexes to turn newly learned
facts into the next bounded queries. Once the remaining moves would be hard
guesses, stop and identify the missing type, field, owner, naming convention, or
other fact that would make progress evidence-backed again.
...
```bash
rg -n "FunctionName|file_name|subsystem" decomp-orchestrator/knowledge/sources/past_prs/data/current/analysis/human_pr_text.md decomp-orchestrator/knowledge/sources/past_prs/data/current/analysis/review_comments.md decomp-orchestrator/knowledge/sources/past_prs/data/prs/index.jsonl
rg -n "PAD_STACK|inline|register|regalloc|FPR|local" decomp-orchestrator/knowledge/sources/past_prs/data/current/analysis/review_comments.md decomp-orchestrator/knowledge/sources/past_prs/data/prs/known_fixes.md
rg -n "sdata2|literal|ordering|fakeFunc|symbols.txt" decomp-orchestrator/knowledge/sources/past_prs/data/current/analysis/review_comments.md decomp-orchestrator/knowledge/sources/past_prs/data/prs/known_fixes.md
rg -n "M2C_FIELD|pointer math|GET_ITEM|union-field" decomp-orchestrator/knowledge/sources/past_prs/data/current/analysis/review_comments.md decomp-orchestrator/knowledge/sources/past_prs/data/prs/known_fixes.md
```

Search by changed file:

```bash
jq 'select(.file=="src/melee/it/items/itlinkbomb.c")' decomp-orchestrator/knowledge/sources/past_prs/data/current/analysis/changed_files.jsonl
```

Search by PR when an example is relevant:

```bash
jq 'select(.pr == 2510)' decomp-orchestrator/knowledge/sources/past_prs/data/current/analysis/text_corpus.jsonl
jq 'select(.pr == 2510 and .change == "add")' decomp-orchestrator/knowledge/sources/past_prs/data/current/analysis/diff_lines.jsonl
```
