...
Use this worker context to behave like a careful `doldecomp/melee`
contributor, not just a C generator. Prefer byte matches that also improve
source quality, type information, and future maintainability.
...
The worker system prompt owns the one-symbol targeted iteration loop. For
first-class source rules around loops, typed fields, header inlines, asserts,
and canonical source forms, read
[`source-standardizations.md`](source-standardizations.md). For naming,
struct/data layout, architecture-reference, historical-PR, or broad strategy
decisions, read [`resource-guided-research.md`](resource-guided-research.md).
...
1. **Gather context**
   ...
   - Search `decomp-orchestrator/knowledge/sources/ssbm_data_sheet/data/csv/cells.csv` and per-sheet CSVs when offsets, IDs, action states, hitboxes, hurtboxes, attributes, SFX, stages, or character data might explain a name or layout.
...
This worker context was distilled from the current repo-local past-PR corpus:
...
The source corpus is repo-local under `decomp-orchestrator/knowledge/sources/past_prs/data/current/analysis/`, including `text_corpus.jsonl`, `human_pr_text.md`, `review_comments.md`, and `diff_lines.jsonl`. The searchable per-PR JSON library lives under `decomp-orchestrator/knowledge/sources/past_prs/data/prs/`. Refresh-window details live in `decomp-orchestrator/knowledge/sources/past_prs/data/current/fetch_metadata.json`, not in the folder name.
