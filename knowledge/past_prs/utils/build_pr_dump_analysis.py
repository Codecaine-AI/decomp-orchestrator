...
def pr_dir(dump_root: Path, prs_dir: str, number: int) -> Path:
    return dump_root / prs_dir / f"pr-{number}"
...
def read_pr_json(
    ...
    for path in (
        raw_json_path(dump_root, number, kind),
        pr_dir(dump_root, prs_dir, number) / RAW_FILE_MAP[kind],
    ):
        if path.exists():
...
def read_diff(dump_root: Path, prs_dir: str, number: int) -> str:
    for path in (
        dump_root / "diffs" / f"{number}.diff",
        pr_dir(dump_root, prs_dir, number) / "diff.diff",
    ):
        if path.exists():
...
def build_dump(
    ...
    *,
    ...
    analysis_dir = dump_root / "analysis"
