...
DEFAULT_DUMP_BASE = PAST_PRS_ROOT
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Maximum PRs to discover in the window.",
    )
    parser.add_argument(
        "--dump-base",
        type=Path,
        default=DEFAULT_DUMP_BASE,
        help="Base directory for the stable dump root. Defaults to decomp-orchestrator/knowledge/past_prs.",
    )
    parser.add_argument(
        "--dump-root",
        type=Path,
        default=None,
        help="Exact dump directory to update. Defaults to decomp-orchestrator/knowledge/past_prs/current.",
    )
    parser.add_argument(
        "--dump-root-name",
...
def discover_prs(
    repo: str, since: dt.date, activity: str, limit: int
) -> list[dict[str, Any]]:
...
def fetch_pr(repo: str, dump_root: Path, number: int) -> None:
    ...
    if proc.returncode == 0:
        ...
        err_path.write_text(proc.stderr or f"gh pr diff exited {proc.returncode}\n", encoding="utf-8")
