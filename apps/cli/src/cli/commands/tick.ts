...
import { directorPrompt, directorQueuedTargets } from "@decomp-orchestrator/agents/director";
...
export async function runDirectorTick(globals: GlobalArgs, args: Map<string, string | true>): Promise<DirectorTickResult> {
  ...
  try {
    ...
    const candidateLimit = Math.max(
      0,
      Math.floor(numberArg(args, "--candidate-limit", globals.project?.dashboard.candidateLimit ?? Math.max(32, run.desiredWorkers * 2))),
    );
    const queueTargetSize = Math.max(
      0,
      Math.floor(numberArg(args, "--queue-target-size", globals.project?.dashboard.queueTargetSize ?? Math.max(candidateLimit, run.desiredWorkers * 2))),
    );
    const candidateWindow = Math.max(
      ...
      Math.max(
        0,
        Math.floor(numberArg(args, "--candidate-window", globals.project?.dashboard.candidateWindow ?? Math.max(candidateLimit, queueTargetSize * 8))),
      ),
    );
