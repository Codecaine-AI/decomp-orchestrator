...
<rules>
    ...
    8. Never run global progress-report refresh commands from a worker,
    ...
    9. Do not stop at the first verified improvement. Treat it as a foothold
       and continue nearby evidence-backed work while budget remains.
    10. For a `matched_code_percent` run, treat exact 100% symbol closure as
    ...
    11. Stop before random guessing. If no evidence-backed next hypothesis
...
<workflow>
    <inputs>
        - `<current_state_json>`: target packet, lease, write set, worker log
          ...
          blockers, including `selected_knowledge_packs`.
    ...
    <phases>
        <phase id="1" name="confirm_scope">
            ...
            <steps>
                ...
                2. Read the selected worker knowledge packs before choosing a
                   tactic. If `sweep_batches` is enabled, read the sweep packs
                   before creating or modifying any `decomp-runs/` artifact.
        ...
        <phase id="2" name="build_evidence_packet">
            ...
            <steps>
                ...
                3. Run or emulate the context helper when useful:
                   `python3 decomp-orchestrator/knowledge/packs/melee-decomp/scripts/decomp_context_lookup.py --target <source_path> --symbol <symbol>`.
        ...
        <phase id="4" name="edit_and_verify">
            <objective>
                Apply scoped edits, verify them, keep improvements, and keep
                working while evidence-backed progress remains.
            </objective>
            <steps>
                ...
                4. After changes that can affect neighbors, check adjacent
                   functions or narrow object/TU objdiff output before
                   claiming progress.
                5. Record every command, exit result, old score, new score,
                   first mismatch key, and artifact path that matters.
                6. If a verified edit improves the score, keep it and continue
                   ...
                   the symbol reaches 100% or the next mismatch has a precise
                   blocker.
                7. If an attempt regresses or produces no useful movement,
                ...
                8. Do not regenerate `build/GALE01/report.json`, run
        ...
        <phase id="5" name="report">
            ...
            <steps>
                ...
                4. If no source changed, keep `edited_paths` empty and explain
...
<reminders>
    ...
    edits. Verify claims with concrete commands. Keep going after a verified
    improvement while the next hypotheses are local and evidence-backed. Stop
    before random guessing and report the blocker or negative evidence. Never
    run global progress-report refresh commands from a worker.
</reminders>
