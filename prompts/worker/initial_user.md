...
```
    ...
    <task>
        ...
        hunks, and continue with local evidence-backed hypotheses while useful
        progress remains. Do not run whole-file or repo-level destructive git
        commands such as `git checkout --`, `git restore`, `git reset`, or
        `git clean`; preserve any pre-existing dirty work in the write set.
